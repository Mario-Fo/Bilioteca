    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6">
      <div class="h-16 flex items-center justify-between">
        ...
        <nav class="hidden lg:flex items-center gap-1" aria-label="Navegación superior">
          ...
          <a href="#libros" class="px-4 py-2 rounded-xl hover:bg-slate-100 font-medium text-slate-700 hover:text-slate-900">
            Libros
          </a>
          <a href="{{ route('categorias.index') }}" class="px-4 py-2 rounded-xl hover:bg-slate-100 font-medium text-slate-700 hover:text-slate-900">
            Categorías
          </a>
          <a href="{{ route('usuarios.index') }}" class="px-4 py-2 rounded-xl hover:bg-slate-100 font-medium text-slate-700 hover:text-slate-900">
            Usuarios
          </a>
          <a href="{{ route('prestamos.index') }}" class="px-4 py-2 rounded-xl hover:bg-slate-100 font-medium text-slate-700 hover:text-slate-900">
            Préstamos
          </a>
          <a href="#reportes" class="px-4 py-2 rounded-xl hover:bg-slate-100 font-medium text-slate-700 hover:text-slate-900">
            Reportes
          </a>
        </nav>
  ...
  <div class="pt-16 min-h-screen flex">
    ...
    <div class="flex-1 lg:ml-0">
      <main class="mx-auto max-w-7xl px-4 sm:px-6 py-6">
        ...
        <section class="mb-6">
          <p class="text-sm text-slate-500">Admin / <span class="text-slate-700 font-medium">Dashboard</span></p>
          <h1 class="mt-2 text-2xl sm:text-3xl font-semibold" id="dashboard">Dashboard</h1>
          <p class="mt-1 text-slate-600">Resumen general y accesos rapidos.</p>
        </section>
        ...
        <section class="mt-6 rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 custom-shadow">
          <div class="flex items-center justify-between gap-3">
            <h2 class="text-lg font-semibold">Libros</h2>
            <a href="{{ route('libros.create') }}"
              class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <table class="min-w-full divide-y divide-slate-200 bg-white text-sm">
              <thead class="bg-slate-50">
                <tr>
                  ...
                  <th class="px-4 py-3 text-left font-semibold text-slate-700">Autor</th>
                  <th class="px-4 py-3 text-left font-semibold text-slate-700">ISBN</th>
                  <th class="px-4 py-3 text-left font-semibold text-slate-700">Categoría</th>
                  <th class="px-4 py-3 text-left font-semibold text-slate-700">Disponibilidad</th>
              ...
              <tbody class="divide-y divide-slate-100">
                ...
                <tr>
                  <td class="px-4 py-3 text-slate-700">{{ $libro->nombre }}</td>
                  <td class="px-4 py-3 text-slate-700">{{ $libro->autor }}</td>
                  <td class="px-4 py-3 text-slate-700">{{ $libro->isbn }}</td>
                  <td class="px-4 py-3 text-slate-700">{{ $libro->categoria->nombre }}</td>
                  <td class="px-4 py-3">
                    <span class="inline-flex rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-medium text-emerald-700">Disponible</span>
                  </td>
                        <td class="px-4 py-3 border-b">
                            <a href="{{ route('libros.edit', $libro->id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded">Editar</a>
                            <form action="{{ route('libros.destroy', $libro->id) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded" onclick="return confirm('¿Estás seguro de eliminar este libro?')">Eliminar</button>
                                </form>
                            </td>
                </tr>
                @endforeach
        ...
        <section class="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-4">
          <article id="libros" class="rounded-3xl border border-slate-200 bg-white p-6 custom-shadow hover-lift">
            <h3 class="text-lg font-semibold">Libros</h3>
            <p class="mt-1 text-slate-600">Gestion de catalogo, altas/bajas, estado y categorias.</p>
          </article>

          <article id="usuarios" class="rounded-3xl border border-slate-200 bg-white p-6 custom-shadow hover-lift">
            <h3 class="text-lg font-semibold">Usuarios</h3>
            <p class="mt-1 text-slate-600">Altas, roles, sanciones y actividad.</p>
          </article>

          <article id="prestamos" class="rounded-3xl border border-slate-200 bg-white p-6 custom-shadow hover-lift">
            <h3 class="text-lg font-semibold">Prestamos</h3>
            <p class="mt-1 text-slate-600">Registro, devoluciones y control de retrasos.</p>
          </article>

          <article id="reportes" class="rounded-3xl border border-slate-200 bg-white p-6 custom-shadow hover-lift">
            <h3 class="text-lg font-semibold">Reportes</h3>
            <p class="mt-1 text-slate-600">Exportaciones, metricas y estadisticas.</p>
          </article>
        </section>
        @endif
      </main>
